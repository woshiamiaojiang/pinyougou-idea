...
app.controller("typeTemplateController",function($scope,$http,$controller,typeTemplateService,brandService,specificationService){
    ...
    //id text,获取所有品牌信息
    $scope.findBrandList=function(){
        brandService.findBrandList().success(function (response) {  //获取到findBrandList方法的值
            $scope.brandList = {
                data:response
            };
        });
    ...
    // }
    $scope.addTableRow=function(){
        $scope.entity.customAttributeItems.push({});   //增加一条空的数据
    }
    ...
    // }
    $scope.deleteTableRow=function(index){
        $scope.entity.customAttributeItems.splice(index,1);   //点击一次移除一条
    }
